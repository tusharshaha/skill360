        ...
        <div className='flex flex-col xl:flex-row items-center gap-5'>
          ...
          <div className="w-full xl:w-2/3 grid sm:grid-cols-2 gap-5 md:px-6">
            {instractors.map((ele, i) => <div key={i} className='flex items-center gap-5'>
              <Image src={ele.img} alt={ele.name} width={200} height={200} className="rounded-full size-[120px] md:size-[150px]" />
              <div className='space-y-2'>
                <h3 className='font-bold'>{ele.name}</h3>
                <p className='text-indigo-500'>{ele.title}</p>
                <p className='flex items-center gap-2 text-slate-500'><FaStar className='text-orange-500' /> (4.8 rating)</p>
